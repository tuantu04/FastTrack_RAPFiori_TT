...
CLASS lhc_booking IMPLEMENTATION.
  ...
  METHOD get_instance_authorizations.
    ...
    READ ENTITIES OF zi_booking_TT01 IN LOCAL MODE
  ...
  METHOD earlynumbering_create.
    " Số lớn nhất hiện tại (Production: dùng Number Range Object).
    SELECT SINGLE FROM zbooking_TT01 FIELDS MAX( booking_id ) INTO @DATA(lv_max_id).
    DATA(lv_num) = COND i( WHEN lv_max_id IS INITIAL THEN 0 ELSE CONV i( lv_max_id+2 ) ).

    " QUAN TRỌNG: phải trả mapped cho MỌI instance (kể cả đã có key),
    " nếu không framework dump CX_CSP_ACT_RESPONSE (no failed / no mapped).
  ...
  METHOD acceptBooking.
    ...
    MODIFY ENTITIES OF zi_booking_TT01 IN LOCAL MODE
      ENTITY Booking
  ...
  METHOD applyDiscount.
    ...
    DATA lt_update TYPE TABLE FOR UPDATE zi_booking_TT01.
    LOOP AT lt_booking INTO DATA(ls).
      " %param mang tham số action (abstract entity ZA_BOOKING_DISC_TT01).
